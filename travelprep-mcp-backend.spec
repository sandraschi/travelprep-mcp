...
a = Analysis(
    ...
    datas=[('src/travelprep_mcp', 'travelprep_mcp')],
    hiddenimports=[
    ...
    excludes=[
        'tkinter', 'setuptools', 'pip', 'wheel', 'test', 'tests',
...
_saved = [
    e for e in a.datas
    if isinstance(e, tuple) and any(k in str(e[0]) for k in _keep_dist)
    and '.dist-info' in str(e[0])
]
for _list in [a.datas, a.binaries, a.zipfiles, a.scripts]:
    _list[:] = [
        e for e in _list
        if not (isinstance(e, tuple) and '.dist-info' in str(e[0]))
    ]
a.datas.extend(_saved)
# SKIP heavy deps -- note: playwright is NOT skipped because the server
# uses it for Booking.com account session and hotelzero scraping.
SKIP = [
    'torch', 'bitsandbytes', 'llvmlite', 'pyarrow', 'pymupdf', 'grpc',
    'numba', 'Cython', 'google', 'azure', 'boto3', 'botocore',
