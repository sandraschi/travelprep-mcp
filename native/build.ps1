...
if (Test-Path $apiFile) {
    ...
    if ($apiContent -match "127.0.0.1:(\d+)") {
        ...
        if ($apiPort -ne $BackendPort) {
            throw "API_BASE in $apiFile points to port $apiPort but backend serves on $BackendPort."
        }
        Write-Host "  API_BASE port: $apiPort (matches backend) v" -ForegroundColor Green
    }
}

# Step 1: Frontend build with TypeScript lint gate
$frontendPath = Join-Path $Root "webapp"
...
# Step 3: Embed backend in Tauri resources with size gate
