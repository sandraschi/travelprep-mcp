...
const BACKEND_NAME: &str = "travelprep-mcp-backend.exe";
const BACKEND_PORT: u16 = 11099;
const BACKEND_TAG: &str = "travelprep-mcp-backend-x86_64-pc-windows-msvc.exe";
const ENV_PORT: &str = "MCP_PORT";
const ENV_HOST: &str = "MCP_HOST";
const ENV_TAURI: &str = "TRAVELPREP_TAURI";
...
fn free_port(port: u16) -> bool {
    #[cfg(windows)]
    {
        ...
        let img_kill = format!(
            "Stop-Process -Name 'travelprep-mcp-backend' -Force -ErrorAction SilentlyContinue; \
             Stop-Process -Name 'travelprep-mcp-native' -Force -ErrorAction SilentlyContinue; \
             taskkill /F /IM travelprep-mcp-backend.exe /T 2>$null; \
             taskkill /F /IM travelprep-mcp-native.exe /T 2>$null"
        );
            ...
            if i == 15 {
                let elevated = format!(
                    "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                     '-NoProfile -Command \"Stop-Process -Name travelprep-mcp-backend -Force -ErrorAction SilentlyContinue; \
                     taskkill /F /IM travelprep-mcp-backend.exe /T 2>$null; \
                     Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue | \
                     ForEach-Object {{ taskkill /F /PID $_.OwningProcess /T 2>$null }}\"'"
                );
