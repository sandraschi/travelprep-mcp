...
discovery. Real tool surface is `search_hotels` (basic) / `find_hotels`
(filtered, 80+ filters, not yet wired here) / `get_hotel_details` /
`compare_hotels` / `check_availability` / `get_reviews` /
`get_price_calendar`.
...
async def price_calendar(
    ...
    """Find the cheapest dates for a given hotel -- not wired into stays.py yet."""
    return await _provider.call_tool(
        "get_price_calendar",
        {
            "hotelUrl": hotel_url,
